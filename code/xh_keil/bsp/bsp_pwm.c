...
void A1PWM_polarity_set(void)
{
    ...
    stcPwmInit.u32StartPolarity = HRPWM4_LA1_START;           // 起始电平设置
...
void A2PWM_polarity_set(void)
{
    ...
    stcPwmInit.u32StartPolarity = HRPWM5_LA2_START;           // 起始电平设置
...
void B1PWM_polarity_set(void)
{
    ...
    /* Configure PWM output */
    stcPwmInit.u32CompareValue = CTRL_PERIOD / 2;               // 默认比较值，50%
    stcPwmInit.u32StartPolarity = HRPWM3_HB1_START;             // 起始电平设置
    stcPwmInit.u32PeakPolarity = HRPWM_PWM_PEAK_HOLD;           // 峰值电平保持
    stcPwmInit.u32ValleyPolarity = HRPWM_PWM_VALLEY_HOLD;       // 谷值电平保持
    ...
    stcPwmInit.u32StartPolarity = HRPWM3_LB1_START;           // 起始电平设置
...
void B2PWM_polarity_set(void)
{
    ...
    stcPwmInit.u32StartPolarity = HRPWM2_LB2_START;           // 起始电平设置
...
void PWM_polarity_set(void)
{
    ...
    stcPwmInit.u32StartPolarity = HRPWM1_HB1_START;             // 起始电平设置
    stcPwmInit.u32PeakPolarity = HRPWM_PWM_PEAK_HOLD;           // 峰值电平保持
    stcPwmInit.u32ValleyPolarity = HRPWM_PWM_VALLEY_HOLD;       // 谷值电平保持
    ...
    stcPwmInit.u32StartPolarity = HRPWM1_LB1_START;           // 起始电平设置
...
void bsp_hrpwm_ValidPeriod_cfg(void)
{
    ...
    HRPWM_SetSpecialCompareAValue(PWM_UNIT, CTRL_PERIOD / 4);     // 专用比较器初始化
    HRPWM_SetSpecialCompareAValue_Buf(PWM_UNIT, CTRL_PERIOD / 4); // 专用比较器缓存初始化
}
...
void bsp_hrpwm_buffer_cfg(void)
{
    ...
    HRPWM_SpecialABufEnable(PWM_UNIT); // 使能专用比较器缓存，只有通道1需要    HRPWM_SpecialBBufEnable(PWM_UNIT);
...
void RAMFUNC bsp_idleA1_process(uint8_t set)
{
    ...
    static uint8_t wait_fresh_CHA = 0; // 每次进入/退出IDLE操作 都等待占空比或极性更新后再进入
...
void bsp_hrpwm_init_pwc(void)
{
    ...
    HRPWM_IntEnable(PWM_UNIT, HRPWM_INT_UP_MATCH_SPECIAL_A); // 使能定时器中断断    HRPWM_IntEnable_Buf(PWM_UNIT, HRPWM_INT_UP_MATCH_SPECIAL_A);
...
// BIT0: HA1   PWM4_A
// BIT1: LA1   PWM4_B
// BIT2: HA2   PWM5_B  //注意此处硬件PWMA/B和H/L相反
// BIT3: LA2   PWM5_A
...
void bsp_pwm_init(void)
{
    ...
    CM_HRPWM2->BPCNAR1 |= (2 << 0) |  /* 10：计数开始时，HRPWM_<t>_PWMA端口输出保持先前状态*/
                          ...
                          (2 << 6) |  /* 10：计数值相等时，HRPWM_<t>_PWMA端口输出不受影响 */
                          ...
                          (2 << 16) | /* 10：下周期开始，HRPWM_<t>_PWMA端口输出设定为低电平。注：该寄存器位可用于实现PWM输出占空比0%至100%的控制*/
    ...
    CM_HRPWM2->BPCNBR1 |= (2 << 0) |  /* 10：计数开始时，HRPWM_<t>_PWMB端口输出保持先前状态*/
                          ...
                          (2 << 6) |  /* 10：计数值相等时，HRPWM_<t>_PWMB端口输出不受影响 */
                          ...
                          (2 << 16) | /* 10：下周期开始，HRPWM_<t>_PWMB端口输出设定为低电平。注：该寄存器位可用于实现PWM输出占空比0%至100%的控制*/
    ...
    bCM_HRPWM_COMMON->GBCONR_b.OSTENU1 = 1; /* 1：单元单次缓存使能 */
...
void bsp_tmr6_pwm_init (void)
{
    ...
    CM_TMR6_1->GCONR |= (0<<0) |        // 0：定时器关闭 1：定时器启动
                        ...
                        (0<<18)         // 00：Z相输入屏蔽功能无效
                                        // 01：位置计数上溢后或下溢后4个计数周期内的Z相输入被屏蔽
                                        // 10：位置计数上溢后或下溢后8个计数周期内的Z相输入被屏蔽
                                        // 11：位置计数上溢后或下溢后16个计数周期内的Z相输入被屏蔽
    ;
    ...
    CM_TMR6_1->PCNAR |= (0<<0) |        // 00：计数开始时，TMR6_<t>_PWMA端口输出设定为低电平
                         ...
                         (0<<16) |       // 0x：设定无效定10：下周期开始，TMR6_<t>_PWMA端口输出设定为低电平
                                         ...
                                         // 注：下周期是指硬件计数模式或锯齿波计数到上溢点或下溢点、三角波计数到谷值
                                         // 注：该寄存器位可用于实现PWM输出占空比0%至100%的控制
                        ...
                        (0<<22) |       // 00 EMBSA[1:0] 选择EMB 事件无效，立即释放TMR6_<t>_PWMA端口（One Shot）
