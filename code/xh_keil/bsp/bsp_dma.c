...
stc_dma_llp_descriptor_t Usart3_tx_List = {0}; //串口3发送DMA链表
stc_dma_llp_descriptor_t Usart3_Rx_List = {0}; //串口3接收DMA链表
...
void DMA_Usart_Rx1_List_Init(void)
{
    ...
    Usart1_Rx_List.SARx = (uint32_t)(&CM_USART1->RDR); //源地址
...
void DMA_Usart_Rx2_List_Init(void)
{
    ...
    Usart2_Rx_List.CHCTLx = DMA_INT_ENABLE | DMA_LLP_ENABLE | DMA_LLP_WAIT | DMA_DATAWIDTH_8BIT \
                           |DMA_RPT_NONE | DMA_NON_SEQ_NONE | DMA_DEST_ADDR_INC | DMA_SRC_ADDR_FIX;  
    Usart2_Rx_List.SARx = (uint32_t)(&CM_USART2->RDR); //源地址
...
void DMA_Usart_Rx3_List_Init(void)
{
    ...
    Usart3_Rx_List.CHCTLx = DMA_INT_ENABLE | DMA_LLP_ENABLE | DMA_LLP_WAIT | DMA_DATAWIDTH_8BIT \
                           |DMA_RPT_NONE | DMA_NON_SEQ_NONE | DMA_DEST_ADDR_INC | DMA_SRC_ADDR_FIX;  
    Usart3_Rx_List.SARx = (uint32_t)(&CM_USART3->RDR); //源地址
    Usart3_Rx_List.DARx = (uint32_t)usart3_rx_buffer; //目的地址
    Usart3_Rx_List.DTCTLx = ((Usart3_RxBuff_Size<<16) | 1); //blocksize:1，stranscnt：Usart1_RxBuff_Size
    Usart3_Rx_List.RPTx = 0; //源地址和目的地址都不重复
    Usart3_Rx_List.SNSEQCTLx = 0; //源地址非连续传输
    Usart3_Rx_List.DNSEQCTLx = 0; //目的地址非连续传输
    Usart3_Rx_List.LLPx = (uint32_t)(&Usart3_Rx_List); //下一个链表地址，指回首地址
}
...
void BSP_DMA_USART3TX_Init(void)
{
    ...
    AOS_SetTriggerEventSrc(AOS_DMA_4, EVT_SRC_USART3_TCI); //触发源为 串口发送完成，每发送完一个字节触发一次 
}
...
void BSP_DMA_USART3RX_Init(void)
{
    ...
    stcDmaInit.u32IntEn = DMA_INT_DISABLE;
    stcDmaInit.u32BlockSize = 1UL;
    stcDmaInit.u32TransCount = Usart3_RxBuff_Size;
    stcDmaInit.u32DataWidth = DMA_DATAWIDTH_8BIT;
    stcDmaInit.u32DestAddr = Usart3_Rx_List.DARx;
    stcDmaInit.u32SrcAddr =  Usart3_Rx_List.SARx;
    stcDmaInit.u32SrcAddrInc = DMA_SRC_ADDR_FIX;
    stcDmaInit.u32DestAddrInc = DMA_DEST_ADDR_INC;
    DMA_Init(USART3_RX_DMA_UNIT, USART3_RX_DMA_CH, &stcDmaInit);

    (void)DMA_LlpStructInit(&stcDmaLlpInit);

    stcDmaLlpInit.u32State = DMA_LLP_ENABLE;
    stcDmaLlpInit.u32Mode  = DMA_LLP_WAIT;
    stcDmaLlpInit.u32Addr  = (uint32_t)&Usart3_Rx_List;   

    (void)DMA_LlpInit(USART3_RX_DMA_UNIT, USART3_RX_DMA_CH, &stcDmaLlpInit);
...
void Dma_Enable(void)
{
    ...
    DMA_ChCmd(USART3_RX_DMA_UNIT, USART3_RX_DMA_CH, ENABLE); //使能DMA通道
}
...
void bsp_dma_init(void)
{
    ...
    BSP_DMA_USART3TX_Init();    //串口1发送   DMA初始化
    BSP_DMA_USART3RX_Init();    //串口1接收   DMA初始化
    Dma_Enable();               //使能DMA
}
