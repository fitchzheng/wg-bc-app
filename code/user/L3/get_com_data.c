...
#define BOOT_DELAY_TICKS_PER_SECOND (10U)
#define BOOT_DELAY_FIXED_PATH_TICKS (6U)

static uint16_t boot_time_delay_get_target_ticks(uint16_t set_boot_time)
{
    uint16_t target_ticks = 0U;

    if(set_boot_time != 0U)
    {
        target_ticks = (uint16_t)(set_boot_time * BOOT_DELAY_TICKS_PER_SECOND);
        if(target_ticks > BOOT_DELAY_FIXED_PATH_TICKS)
        {
            target_ticks = (uint16_t)(target_ticks - BOOT_DELAY_FIXED_PATH_TICKS);
        }
        else
        {
            target_ticks = 0U;
...
{
    static uint16_t TimeDelay = 0U;
    uint16_t target_ticks = boot_time_delay_get_target_ticks(charge_state_data.Boot_Time_Delay.SetBootTime);

    if(target_ticks == 0U)
    {
        TimeDelay = 0U;
        charge_state_data.Boot_Time_Delay.BootTimeDelay = 0U;
        charge_state_data.Boot_Time_Delay.SetBootTimeFlag = 1;
        return;
    }
    if(charge_state_data.Boot_Time_Delay.SetBootTimeFlag == 0)
    {
        if(++TimeDelay >= target_ticks)
        {
            TimeDelay = 0U;
            charge_state_data.Boot_Time_Delay.BootTimeDelay = 0U;
            charge_state_data.Boot_Time_Delay.SetBootTimeFlag = 1;
