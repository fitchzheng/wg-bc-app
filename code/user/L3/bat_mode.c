...
void bat_a_arguments_limi(void)
{
    ...
    if((BatTypeA&0x00FF) < eBAT_SYS_VOLT_MAX)
    {
        ...
        {
            LIMIT_MAX_MIN((float)BAT_MPPT_CURR_MAX_RAW / 100.0f,Bat_Sys_Volt_Config[(BatTypeA&0x00FF)][((BatTypeA&0xff00)>>8)].OutCurrMin,BAT_MPPT_CURR_DEFAULT,get_wg_com_v2_data.com_param.SetInpCurr);
        }
        else
        {
        ...
        if((get_wg_com_v2_data.com_ctrl.SetPowerMode == eMPPT_MODE) ||
           (get_wg_com_v2_data.com_ctrl.MpptSwitch != 0U))
        {
            LIMIT_MAX_MIN(BAT_MPPT_POWER_MAX,Bat_Sys_Volt_Config[(BatTypeA&0x00FF)][((BatTypeA&0xff00)>>8)].OutPowerMin,BAT_MPPT_POWER_DEFAULT,get_wg_com_v2_data.com_param.SetInpCurrPower);
        }
        else
        {
...
void bat_b_arguments_limi(void)
{
    ...
    if((BatTypeB&0x00FF) < eBAT_SYS_VOLT_MAX)
    {
        ...
        if((get_wg_com_v2_data.com_ctrl.SetPowerMode == eMPPT_MODE) ||
           (get_wg_com_v2_data.com_ctrl.MpptSwitch != 0U))
        {
            LIMIT_MAX_MIN((float)BAT_MPPT_CURR_MAX_RAW / 100.0f,Bat_Sys_Volt_Config[(BatTypeB&0x00FF)][((BatTypeB&0xff00)>>8)].OutCurrMin,BAT_MPPT_CURR_DEFAULT,get_wg_com_v2_data.com_param.SetOutCurr);
        }
        else
        {
