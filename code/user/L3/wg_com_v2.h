...
/* 0: power on after default initialization; 1: stay power off until communication writes PowerOnOff=0. */
#ifndef WG_COM_V2_DEFAULT_POWER_ON_OFF
#define WG_COM_V2_DEFAULT_POWER_ON_OFF 0U
#endif
...
#define MODBUS_MIN_FRAME_LEN 5 // 地址(1) + 功能�?1) + 起始寄存�?2) + CRC(2)
...
typedef struct
{
    uint16_t InpVolt;      // A端电�?
    uint16_t InpCurr;      // A端电�?
    uint16_t InpCurrPower; // A端功�?
    uint16_t OutVolt;      // B端电�?
    uint16_t OutCurr;      // B端电�?
    ...
    uint16_t CompensationVoltA; // A端补�?
    uint16_t CompensationVoltB; // B端补�?
    int16_t Temp2;        // 器件温度2
...
typedef struct
{
    ...
    uint16_t SetInpVolt;         // A端电�?
    uint16_t SetInpCurr;         // A端电�?
    uint16_t SetInpCurrPower;    // A端功�?
    uint16_t SetOutVolt;         // B端电�?
    uint16_t SetOutCurr;         // B端电�?
    uint16_t SetOutCurrPower;    // B端功�?
    uint16_t SetInpUvlo;         // A端欠压保�?
    uint16_t SetInpUvloRecover;  // A端欠压保护恢�?
    uint16_t SetInpOVP;          // A端过压保�?
    uint16_t SetInpOVPRecover;   // A端过压保护恢�?
    uint16_t SetOutUvlo;         // B端欠压保�?
    uint16_t SetOutUvloRecover;  // B端欠压保护恢�?
    uint16_t SetOutOVP;          // B端过压保�?
    uint16_t SetOutOVPRecover;   // B端过压保护恢�?
    ...
    uint16_t AuotForwardOpenVoltA; // 自动模式正向A端开启电�?
    uint16_t AuotForwardVeerVoltA; // 自动模式正向转向A电压
    uint16_t AuotForwardShutVoltA; // 自动模式正向A端关闭电�?
    uint16_t AuotReverseOpenVoltB; // 自动模式反向B端开启电�?
    uint16_t AuotReverseShutVoltB; // 自动模式反向B端关闭电�?
